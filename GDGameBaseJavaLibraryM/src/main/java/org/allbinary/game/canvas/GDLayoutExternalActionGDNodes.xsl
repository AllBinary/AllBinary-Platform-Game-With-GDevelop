...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:import href="../GDGameBaseJavaLibraryM/src/main/java/org/allbinary/game/canvas/GDEventCreateAssignGDObjectGDNodeAction.xsl" />
        ...
        <xsl:for-each select="layouts" >
            ...
            <xsl:if test="number($layoutIndex) =
                ...
                public class GD<xsl:value-of select="$layoutIndex" />SpecialAnimationExternalActionGDNodes extends SpecialAnimation
                {
                    ...
                    private static GD<xsl:value-of select="$layoutIndex" />SpecialAnimationExternalActionGDNodes instance;

                    public static GD<xsl:value-of select="$layoutIndex" />SpecialAnimationExternalActionGDNodes getInstance(final AllBinaryGameLayerManager allBinaryGameLayerManager)
                    {
                        instance = new GD<xsl:value-of select="$layoutIndex" />SpecialAnimationExternalActionGDNodes(allBinaryGameLayerManager);
                        return instance;
                    }
