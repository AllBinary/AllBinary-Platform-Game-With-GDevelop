        ...
        <xsl:for-each select="events" >
            ...
            <xsl:for-each select="actions" >
                ...
                <xsl:if test="$typeValue = 'MettreX'" >
                    ...
                    final int size<xsl:value-of select="$name2" /> = globals.<xsl:value-of select="$name2" />GDGameLayerList.size();
                    ...
                    for(int index = 0; index <xsl:text disable-output-escaping="yes" >&lt;</xsl:text> size<xsl:value-of select="$name2" />; index++) {
                        <xsl:value-of select="$name2" /> = ((GDGameLayer) globals.<xsl:value-of select="$name2" />GDGameLayerList.get(index)).gdObject;
