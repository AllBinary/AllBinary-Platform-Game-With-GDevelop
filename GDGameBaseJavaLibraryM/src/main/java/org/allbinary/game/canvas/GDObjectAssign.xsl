    ...
    <xsl:template name="objectsAssign" >
        ...
        <xsl:for-each select="objects" >
            ...
            <xsl:if test="$typeValue = 'Sprite'" >
                ...
                globals.<xsl:value-of select="name" />GDGameLayerFactory = new GDGameLayerFactory(
            ...
            <xsl:if test="$typeValue = 'TextObject::Text'" >
                ...
                */
                ...
                /*
                final GDGameLayer <xsl:value-of select="name" />GDGameLayer = globals.<xsl:value-of select="name" />GDGameLayerFactory.create(globals.<xsl:call-template name="upper-case" ><xsl:with-param name="text" ><xsl:value-of select="name" /></xsl:with-param></xsl:call-template>, (GDObject) <xsl:value-of select="name" />List.get(0), globals.<xsl:value-of select="name" />GDConditionCollidableBehavior);
