...
-->
                        ...
                        public boolean processG(final GDObject gdObject, final BasicArrayList gdGameLayerList, final int gdObjectIndex) {
                            ...
                            try {
                                ...
                                final boolean result = true;
                                    //this.processGPaint(gdObject, null);
                    ...
                    //LinkedObjects::LinkObjects - //Not from parent
...
<!--                    //global name=<xsl:value-of select="name" />-->
                    ...
                    final <xsl:value-of select="$gdObjectFactory" /><xsl:text> </xsl:text><xsl:value-of select="name" /> = (<xsl:value-of select="$gdObjectFactory" />)<xsl:text> </xsl:text><xsl:value-of select="name" />GDGameLayer.gdObject;
                            </xsl:if>
                            </xsl:if>
                            <xsl:if test="name = $parentConditionObject" >
...
<!--                    //name=<xsl:value-of select="name" />-->
                    ...
                    //final GDObject <xsl:value-of select="name" /> = <xsl:value-of select="name" />GDGameLayer.gdObject;
                    <xsl:variable name="gdObjectFactory" >GD<xsl:call-template name="objectFactory" ><xsl:with-param name="name" ><xsl:value-of select="name" /></xsl:with-param><xsl:with-param name="layoutIndex" ><xsl:value-of select="$layoutIndex" /></xsl:with-param></xsl:call-template>GDObjectsFactory.<xsl:value-of select="name" /></xsl:variable>
                    final <xsl:value-of select="$gdObjectFactory" /><xsl:text> </xsl:text><xsl:value-of select="name" /> = ()<xsl:text> </xsl:text><xsl:value-of select="name" />GDGameLayer.gdObject;
                            </xsl:if>
                            </xsl:if>
                            <xsl:if test="name = $parentConditionObject" >
                                //Method argument was already used to skipping - <xsl:value-of select="name" />
