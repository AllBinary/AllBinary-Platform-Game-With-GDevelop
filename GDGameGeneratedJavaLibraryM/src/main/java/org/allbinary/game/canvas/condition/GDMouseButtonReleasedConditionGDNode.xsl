    ...
    <xsl:template name="mouseButtonReleasedConditionGDNode" >
        ...
        <xsl:variable name="thisNodeIndex" select="number(substring(generate-id(), 2) - 65536)" />
                    ...
                    <xsl:variable name="conditionAsString" >Condition nodeId=<xsl:value-of select="generate-id()" /> - <xsl:value-of select="number(substring(generate-id(), 2) - 65536)" /> type=<xsl:value-of select="type/value" /> parameters=<xsl:value-of select="$parametersAsString" /></xsl:variable>
                        ...
                        //MouseButtonReleased - condition - //forExtension=<xsl:value-of select="$forExtension" />
                        ...
                        public boolean process(final MotionGestureEvent motionGestureEvent, final MotionGestureInput lastMotionGestureInput) throws Exception {
                            ...
                            <xsl:if test="not(contains($hasAnotherCondition, 'found'))" >
                                <xsl:for-each select="actions" >
                            //logUtil.put(ACTION_AS_STRING_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" /> + "motion", this, commonStrings.PROCESS);
