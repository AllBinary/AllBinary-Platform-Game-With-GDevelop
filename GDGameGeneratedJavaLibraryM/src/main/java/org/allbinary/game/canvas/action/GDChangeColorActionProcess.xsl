...
<!--                    //objectsGroupsAsString=<xsl:value-of select="$objectsGroupsAsString" />-->
                    ...
                    //ChangeColor - action - index - Change tint of - //forExtension=<xsl:value-of select="$forExtension" />
                    ...
                    <xsl:variable name="firstParamFound" ><xsl:call-template name="firstParamFound" ><xsl:with-param name="nodeId" ><xsl:value-of select="$nodeId" /></xsl:with-param></xsl:call-template></xsl:variable>
                        ...
                        //ChangeColor - <xsl:value-of select="$text" />=<xsl:value-of select="$id" /> - parent or sibling usage <xsl:value-of select="count(//objectsGroups[number(substring(generate-id(), 2) - 65536) &lt; $id])" /> + <xsl:value-of select="count(//objects[number(substring(generate-id(), 2) - 65536) &lt; $id])" />
...
<xsl:text>                        </xsl:text><xsl:value-of select="$paramOneNameObjectsGroups" />GDGameLayer<xsl:for-each select="parameters" ><xsl:if test="position() = 1" >.gdObject.basicColor = smallBasicColorCacheFactory.getAndOrCreate(
                        </xsl:if>
                        <xsl:if test="position() != 1" >basicColorUtil.getARGB(255, 
                            <xsl:call-template name="string-replace-all" >
                                <xsl:with-param name="text" >
...
<!--                        }-->
                    ...
                    <xsl:for-each select="parameters" >
                        <xsl:if test="position() = 1" >
                        <xsl:value-of select="text()" />GDGameLayer.gdObject.basicColor = smallBasicColorCacheFactory.getAndOrCreate(
                        </xsl:if>
                        <xsl:if test="position() != 1" >basicColorUtil.getARGB(255, 
                            <xsl:call-template name="string-replace-all" >
                                <xsl:with-param name="text" >
