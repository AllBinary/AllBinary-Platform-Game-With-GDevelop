...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="animatableCapabilityAnimatableBehaviorSetNameActionProcess" >
        ...
        <xsl:variable name="gdObjectFactory" >GD<xsl:call-template name="objectFactory" ><xsl:with-param name="name" ><xsl:value-of select="$name" /></xsl:with-param><xsl:with-param name="layoutIndex" ><xsl:value-of select="$layoutIndex" /></xsl:with-param></xsl:call-template>GDObjectsFactory.<xsl:value-of select="$name" /></xsl:variable>
                    ...
                    //AnimatableCapability::AnimatableBehavior::SetName - //forExtension=<xsl:value-of select="$forExtension" />
                    ...
                    public boolean process() throws Exception {
                        ...
                        <xsl:for-each select="parameters" >
                            <xsl:if test="position() = 1" >
                                ...
                                if(gdObject.setAnimation(gdObject.getAnimation(</xsl:if>
...
<!--                                    <xsl:variable name="animationName" >
                        ...
                        <xsl:for-each select="parameters" >
                            <xsl:if test="position() = 1" >
                                ...
                                <xsl:if test="position() = last()" >
                                    <xsl:variable name="animationName" >
                                        <xsl:if test="not(contains(text(), 'Variable') or contains(text(), '.'))" >
                                        <xsl:call-template name="upper-case" >
                                            <xsl:with-param name="text" >
                                                <xsl:value-of select="translate(text(), '&quot;', '')" />
                                            </xsl:with-param>
                                        ...
                                        <xsl:if test="contains(text(), 'Variable') or contains(text(), '.')" >
                                            <xsl:value-of select="translate(text(), '&quot;', '')" />
                                        </xsl:if>
                                    </xsl:variable>
                                    <xsl:if test="not(contains(text(), '.'))" ><xsl:call-template name="globals" ><xsl:with-param name="name" ><xsl:value-of select="$name" /></xsl:with-param></xsl:call-template>.</xsl:if>
                    ...
                    //Not from parent - //AnimatableCapability::AnimatableBehavior::SetName
                    public boolean processGD(final GDGameLayer <xsl:value-of select="$firstOrBeforeFourthParam" />GDGameLayer, final GDGameLayer gameLayer2) throws Exception {
                        ...
                        <xsl:if test="not(contains($hasObjectGroup2, 'found'))" >//</xsl:if>final <xsl:value-of select="$gdObjectFactory1" /><xsl:text> </xsl:text><xsl:value-of select="$firstOrBeforeFourthParam" /> = ((<xsl:value-of select="$gdObjectFactory1" />) <xsl:value-of select="$firstOrBeforeFourthParam" />GDGameLayer.gdObject);<xsl:text>&#10;</xsl:text>
                        </xsl:if>
