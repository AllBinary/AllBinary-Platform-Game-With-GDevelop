...
<!--                        <xsl:variable name="gameLayerName" ><xsl:value-of select="$param1" /></xsl:variable>-->
                    ...
                    public boolean processGD(final GDGameLayer[] gameLayerArray) throws Exception {
                        ...
                        try {
                            ...
                            <xsl:for-each select="parameters" >
                                <xsl:if test="position() = 3" ><xsl:if test="text() = '='" ><xsl:value-of select="$param" />GDGameLayer.setText(</xsl:if><xsl:if test="text() = '+'" ><xsl:value-of select="$param" />GDGameLayer.setText(<xsl:value-of select="$param" />GDGameLayer.Text() + </xsl:if></xsl:if>
                                <xsl:if test="position() = 4" >
                                    <xsl:if test="text() = '&quot;&quot;'" >stringUtil.EMPTY_STRING</xsl:if>
                                    <xsl:if test="text() != '&quot;&quot;'" >
                                        <xsl:if test="contains(text(), '&quot;') and not(contains(text(), '+'))" >
