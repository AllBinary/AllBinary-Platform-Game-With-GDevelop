    ...
    <xsl:template name="modVarSceneActionProcess" >
        <xsl:param name="objectsAsString" />
                    ...
                    <xsl:for-each select="parameters" >
                        ...
                        <xsl:variable name="textValue0" ><xsl:if test="contains($textTurnArrayToGet2, '99999999999999999999999')" >Long.MAX_VALUE</xsl:if><xsl:if test="not(contains($textTurnArrayToGet2, '99999999999999999999999'))" ><xsl:value-of select="$textTurnArrayToGet2" /></xsl:if></xsl:variable>
