...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="generateIndexToNodeIdMapping" >
        ...
        <xsl:for-each select="events" >
            ...
            final int NODE_<xsl:value-of select="$id" /> = <xsl:value-of select="$eventCountBefore + $conditionsCountBefore + $actionsCountBefore + $actionParametersCountBefore" />;
            ...
            <xsl:for-each select="actions" >
                ...
                final int PARAM_NODE_<xsl:value-of select="$id" /> = <xsl:value-of select="$eventCountBefore + $conditionsCountBefore + $actionsCountBefore + $actionParametersCountBefore" />;
                </xsl:for-each>
            </xsl:for-each>
