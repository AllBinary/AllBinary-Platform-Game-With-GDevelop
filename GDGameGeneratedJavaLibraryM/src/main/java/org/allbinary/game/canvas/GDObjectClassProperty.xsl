...
-->
                ...
                //public final GPoint <xsl:call-template name="upper-case" ><xsl:with-param name="text" ><xsl:value-of select="name" /></xsl:with-param></xsl:call-template>_POINT = pointFactory.createXY(<xsl:value-of select="x" />, <xsl:value-of select="y" />);
                public final GPoint <xsl:value-of select="name" /> = pointFactory.createXY((int) (<xsl:value-of select="x" /> * 36 / 25 <xsl:if test="not(contains($hasMoreThanOneImageOrRotationDisabled, 'found'))" >* 1.44f</xsl:if>), (int) (<xsl:value-of select="y" /> * 36 / 25 <xsl:if test="not(contains($hasMoreThanOneImageOrRotationDisabled, 'found'))" >* 1.44f</xsl:if>));
                        </xsl:for-each>
                    </xsl:for-each>
                </xsl:for-each>
            </xsl:for-each>
