...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="mouseWheelScrollingDownConditionGDNode" >
        ...
        <xsl:variable name="thisNodeIndex" select="number(substring(generate-id(), 2) - 65536)" />
                    ...
                    //mouseWheelScrollingDownConditionGDNode - //Condition - //IsMouseWheelScrollingDown - GDNode
                    ...
                    <xsl:variable name="conditionAsString" >Condition nodeId=<xsl:value-of select="generate-id()" /> - <xsl:value-of select="number(substring(generate-id(), 2) - 65536)" /> type=<xsl:value-of select="type/value" /> parameters=<xsl:value-of select="$parametersAsString" /></xsl:variable>
                        ...
                        //IsMouseWheelScrollingDown - condition - //forExtension=<xsl:value-of select="$forExtension" />
...
<!--                                //../actions-->
                        ...
                        public boolean processScrolling(final MotionGestureEvent motionGestureEvent, final MotionGestureInput lastMotionGestureInput) throws Exception {
                            ...
                            <xsl:for-each select="conditions" >
                                ...
                                gameGlobals.nodeArray[gameGlobals.NODE_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" />].processScrolling(motionGestureEvent, lastMotionGestureInput);
                                </xsl:if>
                            </xsl:for-each>
                            ...
                            //Event nodeId=<xsl:value-of select="generate-id()" /> - <xsl:value-of select="number(substring(generate-id(), 2) - 65536)" /> position=<xsl:value-of select="position()" /> type=<xsl:value-of select="type" /> <xsl:if test="object" > object=<xsl:value-of select="object" /></xsl:if> <xsl:if test="target" > target=<xsl:value-of select="target" /></xsl:if> disable=<xsl:value-of select="disabled" />
                            //Event - //BuiltinCommonInstructions::Link - call - //MouseButtonReleased
