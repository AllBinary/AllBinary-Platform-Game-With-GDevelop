        ...
        <xsl:for-each select="events" >
            ...
            <xsl:for-each select="actions" >
                ...
                <xsl:if test="$typeValue = 'MettreX' or $typeValue = 'SetX'" >
                    ...
                    //MettreX
                    if(true) {
                    final int size<xsl:value-of select="$name2" /> = <xsl:call-template name="globals" ><xsl:with-param name="name" ><xsl:value-of select="$name2" /></xsl:with-param></xsl:call-template>.<xsl:value-of select="$name2" />GDGameLayerList.size();
                    ...
                    for(int index = 0; index <xsl:text disable-output-escaping="yes" >&lt;</xsl:text> size<xsl:value-of select="$name2" />; index++) {
                        <xsl:value-of select="$name2" /> = (GDObject) ((GDGameLayer) <xsl:call-template name="globals" ><xsl:with-param name="name" ><xsl:value-of select="$name2" /></xsl:with-param></xsl:call-template>.<xsl:value-of select="$name2" />GDGameLayerList.get(index)).gdObject;
                        gameGlobals.nodeArray[gameGlobals.NODE_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" />].processGPaint(<xsl:value-of select="$name2" />, null, graphics);
                    }
                    }
                    </xsl:if>
                    <!--
