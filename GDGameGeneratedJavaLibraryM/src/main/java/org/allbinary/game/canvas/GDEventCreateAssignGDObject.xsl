    ...
    <xsl:template name="gdObjectForParam" >
        ...
        //gameLayer=<xsl:value-of select="$gameLayer" />            
            ...
            if(<xsl:value-of select="$name2" /> == null) {
                ...
                return <xsl:if test="contains($caller, 'createActionProcess') or (contains($caller, 'hackProcessing2') or contains($caller, 'externalEventsCreateAssignGDObject - //sceneVariableConditionGDNode'))" ><xsl:if test="not(contains($caller, 'conditionLayout - //eventsCreateAssignGDObject - //hackProcessing2'))" >true</xsl:if></xsl:if>;
            }
        </xsl:if>                                    
    ...
    <xsl:template name="actionsProcessing" >
        ...
        <xsl:param name="parentParam2" />
                        ...
                        <xsl:for-each select="actions" >
                            ...
                            <xsl:when test="$nodeId > number(substring(generate-id(), 2) - 65536)" >
...
<!--                                //totalParams=<xsl:value-of select="$totalParams" /> <xsl:value-of select="number($totalParams)" /> <xsl:if test="number($totalParams) = 1" >Exactly 1 param</xsl:if><xsl:if test="number($totalParams) > 1" >More than 1 param</xsl:if>-->
    ...
    <xsl:template name="actionsProcessHack" >
        <xsl:param name="caller" />
        <xsl:param name="layoutIndex" />
        <xsl:param name="objectsGroupsAsString" />
        <xsl:param name="objectsAsString" />
        <xsl:param name="parametersAsString" />
                            ...
                            //caller=<xsl:value-of select="$caller" /> - //actionProcess - //hackProcessing - //Action - //Create - call
                            //TWB is this still needed2
    ...
    <xsl:template name="eventsCreateProcessUsedHack" >
        <xsl:param name="caller" />
        ...
        <xsl:if test="contains($hasCollisionProcessGD, 'found')" >
            ...
            <xsl:for-each select="conditions" >
                ...
                <xsl:if test="number($thisNodeIndex) != number(substring(generate-id(), 2) - 65536)" >
                <!-- //caller=<xsl:value-of select="$caller" /> - //eventsCreateProcessUsed -->
                <xsl:if test="type/value != 'DepartScene' and type/value != 'SceneJustBegins' and type/value != 'Timer' and (not($caller = 'conditionLayout - //eventsCreateAssignGDObjectGDNodesCondition' and type/value = 'NbObjet'))" >
                <xsl:variable name="parametersAsString0" ><xsl:for-each select="parameters" ><xsl:value-of select="text()" />,</xsl:for-each></xsl:variable>
                <xsl:variable name="parametersAsString" ><xsl:value-of select="translate(translate($parametersAsString0, '&#10;', ''), '\&#34;', '')" /></xsl:variable>
            ...
            <xsl:for-each select="actions" >
                ...
                <xsl:if test="$typeValue = 'Delete'" >
                    ...
                    if(<xsl:value-of select="$gameLayerName" />GDGameLayer != null) {
                        gameGlobals.nodeArray[gameGlobals.NODE_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" />].processGD(<xsl:value-of select="$gameLayerName" />GDGameLayer, null);
                    } else {
            ...
            <xsl:for-each select="actions" >
                ...
                <xsl:if test="$typeValue = 'AddForceAL'" >
                        ...
                        for(int index = 0; index <xsl:text disable-output-escaping="yes" >&lt; </xsl:text> <xsl:value-of select="$gameLayerName" />Size; index++) {
                            ...
                            if(<xsl:value-of select="$gameLayerName" />GDGameLayer != null) {
                                gameGlobals.nodeArray[gameGlobals.NODE_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" />].processGD(<xsl:value-of select="$gameLayerName" /><xsl:value-of select="$typeValue" />GameLayerAtIndex, null);
                            }
                            ...
                            gameGlobals.nodeArray[gameGlobals.NODE_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" />].processGD(gameLayer, null);
            ...
            <xsl:for-each select="actions" >
                ...
                <xsl:if test="$typeValue = 'AddForceAL'" >
                    ...
                    -->
                            ...
                            gameGlobals.nodeArray[gameGlobals.NODE_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" />].processGD((GDGameLayer) <xsl:call-template name="globals" ><xsl:with-param name="name" ><xsl:value-of select="$gameLayerName" /></xsl:with-param></xsl:call-template>.<xsl:value-of select="$gameLayerName" />GDGameLayerList.get(index), null);
                </xsl:if>
