...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="setSceneVariableAsBooleanActionProcess" >
        ...
        <xsl:variable name="name" ><xsl:for-each select="parameters" ><xsl:if test="position() = 1" ><xsl:value-of select="text()" /></xsl:if></xsl:for-each></xsl:variable>
                    //SetBooleanVariable - was //SetSceneVariableAsBoolean - action - //forExtension=<xsl:value-of select="$forExtension" />
                    ...
                    public boolean processGD(final GDGameLayer[] gameLayerArray) throws Exception {
                        ...
                        <xsl:for-each select="parameters" ><xsl:if test="position() = 1" ><xsl:value-of select="text()" /> = </xsl:if><xsl:if test="position() = 2" ><xsl:call-template name="lower-case" ><xsl:with-param name="text" ><xsl:value-of select="text()" /></xsl:with-param></xsl:call-template></xsl:if><xsl:if test="position() = last()" >;</xsl:if></xsl:for-each>
