        ...
        <xsl:for-each select="events" >
            ...
            <xsl:variable name="foundTimerCondition" >
                ...
                //private final String ACTION_AS_STRING_<xsl:value-of select="number(substring(generate-id(), 2) - 65536)" /> = "C: <xsl:value-of select="translate($actionAsString, $quote, ' ')" />";
                        ...
                        <xsl:if test="$typeValue != 'ModVarScene' and $typeValue != 'AddForceAL' and $typeValue != 'PlayMusicCanal' and $typeValue != 'StopMusicCanal' and $typeValue != 'PlaySoundCanal' and $typeValue != 'StopSoundCanal'" >
            ...
            <!-- //foundOtherCondition=<xsl:value-of select="$foundOtherCondition" /> -->
                ...
                <xsl:if test="not(contains($foundOtherCondition, 'found') or contains($foundTimerCondition, 'found') or (contains($foundVarSceneCondition, 'found') and contains($foundLinkEvent, 'found')))" >
                    ...
                    final BasicArrayList gdGameLayerList = (BasicArrayList) <xsl:call-template name="globals" ><xsl:with-param name="name" ><xsl:value-of select="$object" /></xsl:with-param></xsl:call-template>.<xsl:value-of select="$object" />GDGameLayerListOfList.get(size3);
                    </xsl:if>
