...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="fileSystemLoadStringFromFileSyncActionProcess" >
        ...
        <xsl:variable name="name" ><xsl:for-each select="parameters" ><xsl:if test="position() = 1" ><xsl:value-of select="text()" /></xsl:if></xsl:for-each></xsl:variable>
                    //FileSystem.LoadStringFromFileSync - was ? - action - //forExtension=<xsl:value-of select="$forExtension" />
                    ...
                    public boolean process() throws Exception {
                        ...
                        <xsl:value-of select="$param1" /> = FileSystem.LoadStringFromFileSync(<xsl:value-of select="$param2" />);
