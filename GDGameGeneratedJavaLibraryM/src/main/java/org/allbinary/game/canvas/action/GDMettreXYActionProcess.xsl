                    ...
                    final GDObject <xsl:value-of select="$gameLayerName" /> = <xsl:value-of select="$gameLayerName" />GDGameLayer.gdObject;
                        ...
                        <xsl:if test="contains($hasObjectVariable, 'found')" >
                            ...
                            //hasObjectVariable - found
                                ...
                                <xsl:variable name="mapPositionHackX" ><xsl:for-each select="parameters" ><xsl:if test="position() = 3" ><xsl:if test="contains(text(), 'startX')" > &#42; 1.44f</xsl:if></xsl:if></xsl:for-each></xsl:variable>
                                <xsl:variable name="mapPositionHackY" ><xsl:for-each select="parameters" ><xsl:if test="position() = 5" ><xsl:if test="contains(text(), 'startY')" > &#42; 2.07f</xsl:if></xsl:if></xsl:for-each></xsl:variable>
