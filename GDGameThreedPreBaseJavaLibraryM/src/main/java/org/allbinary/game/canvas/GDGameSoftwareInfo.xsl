...
public class GDGameSoftwareInfo
{
    ...
    new SoftwareInformation("<xsl:value-of select="properties/packageName" />.GDGameThreed", "1.2.0", <xsl:if test="string-length($shortName) > 8" >Short Name is to long</xsl:if>"<xsl:value-of select="$shortName" />");
