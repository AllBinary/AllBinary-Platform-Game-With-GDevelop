 ...
 * @author User
 */
public class GDListObjectsWithoutBehavior extends GDJSONGeneratorBase {

    private final FileUnamedUtil fileUnamedUtil = FileUnamedUtil.getInstance();

    private final String FIND_BEHAVIOR = "PathfindingBehavior::PathfindingBehavior";

    public GDListObjectsWithoutBehavior() {
    }
    ...
    private void process(final GDObject gdObject) {
        ...
        if(found) {
            ...
            System.out.println("GDObject with Missing Behavior: " + gdObject.name);
        }
    }
